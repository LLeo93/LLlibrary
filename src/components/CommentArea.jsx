...
const CommentArea = ({ bookId, onNewComment }) => {
  ...
  const submitComment = (e) => {
    ...
    const newComment = {
      comment,
      rate: rating,
      elementId: bookId, // Assicurati che il bookId venga passato correttamente
    };
    ...
    fetch(`${ADD_COMMENT_URL}${bookId}`, {
      ...
      .then((response) => {
        if (!response.ok) {
          // Gestione degli errori con codice 400 (Bad Request)
      ...
      .then(() => {
        ...
        setComment('');
        setRating(1);
        // Chiamata per ricaricare i commenti
        onNewComment();
      })
      .catch((error) => {
